...
def filtered_unique_mean(prices: Iterable) -> Optional[Decimal]:
    ...
    # Используем более строгий коэффициент для отсечения выбросов
    lower = Q1 - Decimal("1.5") * IQR
    upper = Q3 + Decimal("1.5") * IQR

    # Дополнительная защита: если нижняя граница отрицательная, устанавливаем ее в 0
    if lower < 0:
        lower = Decimal("0")
    ...
    # Если после фильтрации осталось мало значений, используем медиану
    if len(filtered) < 2:
        med = median(unique_nums)
        # Берем значения в пределах 3 стандартных отклонений от медианы
...
#     IQR = Q3 - Q1
#
#     # Используем стандартный коэффициент 1.5 вместо 0.5
#     lower = Q1 - Decimal("1.5") * IQR
#     upper = Q3 + Decimal("1.5") * IQR
#
#     # Дополнительная защита: если нижняя граница отрицательная, устанавливаем ее в 0
...
#     filtered = [n for n in unique_nums if lower <= n <= upper]
#
#     # Если после фильтрации осталось меньше 2 значений, используем медиану
#     if len(filtered) < 2:
#         med = median(unique_nums)
#         # Берем значения в пределах 3 стандартных отклонений от медианы
#         from statistics import stdev
